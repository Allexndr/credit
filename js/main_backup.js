...
class Forms {
    ...
    sendToWhatsApp(form, formData) {
        ...
        // Маппинг смайликов для типов кредитов (совместимые с WhatsApp)
        ...
        // Маппинг полей для красивого отображения (простые эмодзи)
        const fieldLabels = {
            'name': '👤 Имя',
            'phone': '� Телефон',
            'email': '📧 Email',
        ...
        if (creditTypeValue && creditTypeEmojis[creditTypeValue]) {
            message = message.replace('🔔 НОВАЯ ЗАЯВКА НА КРЕДИТ', 
                `${creditTypeEmojis[creditTypeValue]} ЗАЯВКА: ${creditTypeText.toUpperCase()}`);
        }
